...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=10 cm]{v_bd9_3_up_tau_add_new_rule}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=10 cm]{v_bd9_4_up_tau_add_new_rule}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=10 cm]{v_bd9_4_up_mu_add_new_rule}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=10 cm]{v_bd9_4_up_sigma_add_new_rule}
...
\end{figure}
%与图12中变化点t1后验样本相比，图17所示中后验样本分布更加有利于变化点的识别。
%尽管在1200秒附近还存在一些样本点，但几乎微乎其微，基本都在1000以下。
%t2的后验分布出现了些许变化，在1750s出现了少许样本点，不足以影响变化点的识别。
%从图13和图18的对比中，可以看出均值后验分布的次峰基本消失。
%但是依然存在，为了减小这些样本点的影响，采用样本的中位数作为均值的最终估值，得到的均值分别为1,2,3.
%利用这些均值计算的位移分别为4,5.
%而表格2中测量得到的位移分别为7,8.
%主要是由于。。引起，从图2中可以看出存在明显浮动。
%同理，从图19和图14的对比中，可以看出方差估值的次峰得到了显著削弱。
%从以上的讨论中，可以看出添加的先验能够有效的提升辩护点的可识别性。
%另一方面也说明，本文提出的基于贝叶斯推理方法足够灵活，可以根据需求添加先验的方式满足不同的分析需求。
...
\section{Conclusions}
\label{concl}
This study set out to develop a Bayesian model for displacement identification and extraction. 
The second aim of this study was to investigate the effects of various parameters and models on the performance of the proposed algorithm. 
State models and measurement models under different state parameters are designed for the IMM-KF algorithm, and the verification schemes based on vision and segmental integration are proposed.
Four sets of experiments were carried out to verify the effectiveness of the designed algorithm. 
These experiments confirmed the effectiveness of the algorithm in displacement extraction and vibration detection.

For obvious vibration, the IMM Kalman filter based on the CP + DV model set can be effectively performed for online displacement extraction and vibration detection. The CV, CA, and CJ models are not conducive to displacement extraction due to the displacement drift, although this drift can be suppressed by the virtual measurement model. For slight vibration, the performance of the IMM Kalman filter with CP + DV model set is poor. But the performance of the IMM-SIS with the CP+CA model set is good. These findings have significant implications for the understanding of how to carry out online displacement extraction and vibration detection. This study provides the first comprehensive assessment of interactive multiple models for displacement extraction and vibration detection in SHM.
