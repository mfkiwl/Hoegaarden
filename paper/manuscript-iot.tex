...
For the data collected in this experiment,  the coordinate time series is obtained by RTKLIB with the same processing strategy as the previous experiment, and the up component of the time series is shown in Fig. \ref{fig_v_bd9_4_up_ts}. 
...
The time series are processed by the proposed model, and Bayesian inference is implemented by MCMC sampling. The results are shown in Fig. \ref{fig_v_bd9_4_up_tau}, \ref{fig_v_bd9_4_up_mu}, and \ref{fig_v_bd9_4_up_sigma}.
...
The interval between adjacent change points is constraint by adding a prior. 
...
\end{equation}
where $d$ is the shortest time interval to be constraint. The new Bayesian probability model with the prior is shown in Fig. \ref{fig_v_simple_p_model_add_new_rule}.
